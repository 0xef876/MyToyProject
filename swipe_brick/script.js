(() => {
	...
	high_text = document.querySelector(".high_score");
	high_text.innerText = "High Score = " + high_score;
	...
	const callback = async () => { // 공 날라갔다가 돌아왔을때
		...
		// }
		/* //ball */
		let current_score = turn - 1;
		document.querySelector('.score').innerText = "Score = " + current_score;
		...
		document.querySelector(".b").innerText = "Ball x " + Balls.length;
		Blocks = Blocks.map(v => {
			if (v.opacity != 1) v.opacity = 1;
			return v;
		});
	...
	const end = () => { // 게임 끝났을때
		...
		Swal.fire({
			...
			Swal.fire({
				...
				preConfirm: (username) => {
					// post 요청
					return fetch(`https://kittypark.duckdns.org:8443/rank?username=${username}&score=${turn}`,
					{method: "POST"})
